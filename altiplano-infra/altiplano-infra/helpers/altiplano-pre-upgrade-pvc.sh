...
usage(){
   ...
   echo "Usage: ./altiplano-pre-upgrade-pvc.sh -r <SOLUTION_RELEASE_NAME> -n <namespace>"
   echo "Example:"
   echo "  ./altiplano-pre-upgrade-pvc.sh -r nokia -n my_namespace"
   echo ""
...
declare -a PVCs=(
                ''${RELEASE}'-av-stagedfiles-pvclaim'
...
patch_PVs() {
 ...
 for (( i=0; i<$pvc; i++)); do
   ...
   if [[ ${PVCs[${i}]} == $PVC_WEBDAV_CBURTMP ]]; then
     ...
     kubectl patch pv $pv_item -p '{"spec":{"claimRef": {"name":"'${RELEASE}'-av-stagedfiles-pvclaim", "namespace":"'$NAMESPACE'"} }}'
   fi
 done
}
