...
----
spec:
{{ include "csf-common-lib.v1.certificateValues" (tuple . .Values.echoserver.certificate .Values.echoserver) | indent 2 }}
----
...
*/}}
{{- define "csf-common-lib.v1.certificateValues" -}}
{{- $root := index . 0 }}
{{- $certificate := index . 1 }}
{{- $workload := index . 2 }}
{{- $defaultIssuerName := include "csf-common-lib.v1.resourceName" (tuple $root "Issuer" $workload.nameSuffix) }}
secretName: {{ include "csf-common-lib.v1.certificateSecretName" (tuple $root $certificate) }}
issuerRef:
...
dnsNames:
  - localhost
  - {{ include "csf-common-lib.v1.resourceName" (tuple $root "Service" $workload.nameSuffix) }}.{{ $root.Release.Namespace }}
  - {{ include "csf-common-lib.v1.resourceName" (tuple $root "Service" $workload.nameSuffix) }}.{{ $root.Release.Namespace }}.svc
  - {{ include "csf-common-lib.v1.resourceName" (tuple $root "Service" $workload.nameSuffix) }}.{{ $root.Release.Namespace }}.svc.{{ $root.Values.clusterDomain | default "cluster.local" }}
{{- end }}
{{- if $certificate.uris }}
uris:
...
{{/*
**DEPRECATED** - use `csf-common-lib.v3.certificateSecretName` instead
*/}}
{{- define "csf-common-lib.v1.certificateSecretName" -}}
{{- $root := index . 0 }}
{{- $certificate := index . 1 }}
{{- include "csf-common-lib.v1.resourceName" (tuple $root "Secret" ($certificate.nameSuffix | default "cert")) }}
{{- end }}