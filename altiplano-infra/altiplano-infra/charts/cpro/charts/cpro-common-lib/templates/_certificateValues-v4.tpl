...
*/}}
{{- define "cpro-common-lib.v4.certificateValues" -}}
{{- $root := index . 0 }}
{{- $workload := index . 1 }}
{{- $suffix := index . 2 }}
...
{{- include "cpro-common-lib.v4.issuerRefDetails" (tuple $root $workload $suffix) }}
...
{{- else if and ($root.Values.certManager.enabled) ($certificate.enabled) }}
usages:
{{- if not (empty $root.Values.certManager.usages) }}
{{ toYaml $root.Values.certManager.usages | indent 2 }}
{{- end }}
  - server auth
  - client auth
{{- else }}
...
{{- else if and ($root.Values.certManager.enabled) ($certificate.enabled) }}
dnsNames:
{{- if not (empty $root.Values.certManager.dnsNames) }}
{{ toYaml $root.Values.certManager.dnsNames | indent 2 }}
{{- end }}
  - localhost
  - {{ include "csf-common-lib.v3.resourceName" (tuple $root "Service" $workload.nameSuffix) }}.{{ $root.Release.Namespace }}
  - {{ include "csf-common-lib.v3.resourceName" (tuple $root "Service" $workload.nameSuffix) }}.{{ $root.Release.Namespace }}.svc
...
{{- else if and ($root.Values.certManager.enabled) ($certificate.enabled) }}
ipAddresses:
{{- if not (empty $root.Values.certManager.ipAddresses) }}
{{ toYaml $root.Values.certManager.ipAddresses | indent 2 }}
{{- end }}
  - "127.0.0.1"
  - "::1"
{{- else }}
...
{{- define "cpro-common-lib.v4.issuerRefDetails" -}}
{{- $root := index . 0 }}
{{- $workload := index . 1 }}
{{- $suffix := index . 2 }}
{{- $workloadCertRef := $workload.certificate.issuerRef }}
{{- $rootCertRef := $root.Values.certManager.issuerRef }}
{{- $globalCertRef := $root.Values.global.certManager.issuerRef }}
{{- $defaultIssuerName := include "csf-common-lib.v3.resourceName" (tuple $root "Issuer" $suffix) }}
{{- $issuerRefName := coalesce $workloadCertRef.name $rootCertRef.name $globalCertRef.name $defaultIssuerName  }}
issuerRef:
  name: {{ $issuerRefName }}
{{- if eq $issuerRefName $defaultIssuerName }}
  kind: {{ "Issuer" }}
  group: {{ "cert-manager.io" }}
{{- else }}
  kind: {{ coalesce $workloadCertRef.kind $rootCertRef.kind $globalCertRef.kind }}
  group: {{ coalesce $workloadCertRef.group $rootCertRef.group $globalCertRef.group }}
{{- end }}
{{- end }}
