...
declare NAMESPACE=$1
declare SRC_STS_FLU=$2
declare DST_STS_FLU=$3
...
if [ -z "$SRC_STS_FLU" ]; then
  echo "Source fluentd statefulset name not found!!"
  exit 1
fi

if [ -z "$DST_STS_FLU" ]; then
  echo "Destination fluentd statefulset name not found!!"
  exit 1
fi
...
function migrate_stateful_set_pvcs() {
  ...
  if [ $SOURCE_STATEFUL_SET_NAME != $DESTINATION_STATEFUL_SET_NAME ]; then
    for templateName in ${VOLUME_CLAIM_TEMPLATE_NAMES};
    do
      log_info "Migrating PVCs for volume claim template name '${templateName}'"
      migrate_statuful_set_pvcs_for_template_name "${SOURCE_STATEFUL_SET_NAME}" "${DESTINATION_STATEFUL_SET_NAME}" "${templateName}"
    done
  else
...
echo "Will migrate Fluentd PVC from StatefulSet '$SRC_STS_FLU' to '$DST_STS_FLU' in namespace '$NAMESPACE'."
rename_stateful_set_pvcs "$SRC_STS_FLU" "$DST_STS_FLU"
