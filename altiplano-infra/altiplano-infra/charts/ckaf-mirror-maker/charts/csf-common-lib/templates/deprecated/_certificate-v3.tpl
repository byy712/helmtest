...
{{- if and $workload.tls.enabled (eq (include "csf-common-lib.v1.coalesceBoolean" (tuple .Values.certManager.enabled .Values.global.certManager.enabled true)) "true") }}
  {{- include "csf-common-lib.v3.certificate" (tuple . $workload) }}
{{- end }}
----
...
*/}}
{{- define "csf-common-lib.v3.certificate" -}}
{{- $root := index . 0 }}
{{- $workload := index . 1 }}
...
spec:
{{ include "csf-common-lib.v3.certificateValues" (tuple $root $workload.certificate $workload) | indent 2 }}
{{- end }}
