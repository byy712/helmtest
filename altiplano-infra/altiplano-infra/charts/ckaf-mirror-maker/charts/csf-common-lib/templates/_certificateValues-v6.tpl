...
### [v6]
#### Changed
* Fix `subject` field generation
* Simplified implementation for `ipAddresses` field
...
*/}}
{{- define "csf-common-lib.v6.certificateValues" -}}
{{- $root := index . 0 }}
{{- $workload := index . 1 }}
...
subject: {{ $certificate.subject | toYaml | nindent 2 }}
{{- end }}
...
ipAddresses: {{- $certificate.ipAddresses | default $defaultIpAddress | toYaml | nindent 2 }}
{{- else if not (contains "skipKeys" $behaviorFlavor)}}
ipAddresses: {{- $defaultIpAddress | toYaml | nindent 2 }}
{{- end}}
{{- end }}
