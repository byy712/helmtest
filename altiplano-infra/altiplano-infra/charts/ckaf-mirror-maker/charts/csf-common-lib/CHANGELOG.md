...
### Added
- `csf-common-lib.v3.podAntiAffinity`:
  - add support for `namespaces` list and `namespaceSelector`
  - add `customMatchLabels` argument to pass custom `labelselectors`
- _unifiedLogging.volumes-v3.tpl file with new `csf-common-lib.v3.unifiedLogging.volumes` function.
  - `csf-common-lib.v3.unifiedLogging.volumes` Changed merge function usage so global settings don't overwrite workload ones.
- _unifiedLogging.configMap-v3.tpl file with new `csf-common-lib.v3.unifiedLogging.configMap` function.
  - `csf-common-lib.v3.unifiedLogging.configMap` Changed merge function usage so global settings don't overwrite workload ones.
- _unifiedLogging.configMap-v2.tpl file with new `csf-common-lib.v2.unifiedLogging.volumeMounts` function.
  - `csf-common-lib.v2.unifiedLogging.volumeMounts` Changed merge function usage so global settings don't overwrite workload ones.
- _unifiedLogging.config-v2.tpl file with new `csf-common-lib.v2.unifiedLogging.config` function.
  - `csf-common-lib.v2.unifiedLogging.config` Changed merge function usage so global settings don't overwrite workload ones.

### Deprecated
- `csf-common-lib.v2.unifiedLogging.configMap` has issues with global vs workload priorities. Use `csf-common-lib.v3.unifiedLogging.configMap` instead
- `csf-common-lib.v2.unifiedLogging.volumes` has issues with global vs workload priorities. Use `csf-common-lib.v3.unifiedLogging.volumes` instead
- `csf-common-lib.v1.unifiedLogging.config` has issues with global vs workload priorities. Use `csf-common-lib.v2.unifiedLogging.config` instead
- `csf-common-lib.v1.unifiedLogging.volumeMounts` has issues with global vs workload priorities. Use `csf-common-lib.v2.unifiedLogging.volumeMounts` instead

## [1.14.0] - 2024-02-01

### Added
- `csf-common-lib.v1.resourcesContainer` - Introduced a new template which extracts values for resource container requests and limits. Will be satisfying all resources requirement `HBP_Kubernetes_Pod_res_1`, `HBP_Kubernetes_Pod_res_3`, and `HBP_Kubernetes_Pod_res_5` at once.
- `csf-common-lib.v6.certificateValues` and `csf-common-lib.v6.certificate` - fix `subject` field generation
