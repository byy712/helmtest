...
case $DATA_BLOCK_ENCODING in
  (NONE|PREFIX|DIFF|FAST_DIFF|ROW_INDEX_V1)  :;;
  (*)
...
echo "alter ttl"
ttl=$( echo "describe '$TSDB_TABLE'" | $HBASE_PREFIX/bin/hbase shell -n )
echo "$ttl"
if [[ $ttl == *"$TSDB_TTL"* ]];
then
  echo "values are same no need to alter $TSDB_TABLE"
else
  echo "values are not same, alter $TSDB_TABLE"
  ##updateTsdbTableTTL
  exec "$HBASE_PREFIX/bin/hbase" shell <<EOF
  alter '$TSDB_TABLE', NAME => 't', TTL => $TSDB_TTL
EOF
  echo "alter $TSDB_TABLE done"
fi
