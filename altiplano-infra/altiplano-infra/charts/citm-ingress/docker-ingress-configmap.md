...
|[map-hash-max-size](#map-hash-max-size)|int|2048|
...
|[enable-vts-status](#enable-vts-status)|"false"|
|[vts-status-zone-size](#vts-status-zone-size)|"10m"|
...
Sets the max size for the [map variables hash tables](http://nginx.org/en/docs/http/ngx_http_map_module.html#map_hash_max_size). The details of setting up hash tables are provided in a separate [document](http://nginx.org/en/docs/hash.html).

Applied also on [stream](http://nginx.org/en/docs/stream/ngx_stream_map_module.html)
...
nginx variables that can be used are listed [here](http://nginx.org/en/docs/varindex.html)
...
An address can also be retrieved from an environemnt variable. In that case, suffix it using `env.`

For example, by default POD_IP is automaticcaly set in citm ingress controller. If you want to listen also on internal pod ip, simply provide `env.POD_IP` in the list
...
controller:
  ...
  httpsPort: 449
  statusPort: 8085
  bindAddress: 10.76.75.7,env.POD_IP
```
