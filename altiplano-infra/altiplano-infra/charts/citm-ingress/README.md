...
--------- | ----------- | ------- 
global.registry | default ingress controller container image registry | csf-docker-delivered.repo.cci.nokia.net
global.flatRegistry | Set flatRegistry to true to use images from registries with a flat structure | false
...
global.containerNamePrefix |ability to prefix ALL container names with prefixes. Note that release-name can be truncated up to 12 chars when podNamePrefix is used. Helm release name can be up to 43 chars and also first 12 chars should be meaningful. | ""
...
global.podSecurityPolicy.pspName | Use the custom/existing PSP, need to set global.podSecurityPolicy.userProvided needs to true to use this | ""
global.dualStack.enabled | enable or disable dual stack | false
global.dualStack.ipFamilyPolicy | set the behavior of dual stack among SingleStack, PreferDualStack and RequireDualStack | ''
global.dualStack.ipFamilies | set the ip families to be assigned to resource | []
...
global.imageFlavor | set image flavor at global level, available options "rocky8" | ""
global.imageFlavorPolicy | set image flavor policy at global level, available options - "Strict, BestMatch" | ""
global.certManager.enabled | Use cert-manager service for generating default certificate (precedence order .Values.certManager.enabled --> .Values.certManager.used --> .Values.global.certManager.enabled ) | true
global.certManager.issuerRef.name | provide certManager issuer name | ""
global.certManager.issuerRef.kind | provide certManager issuer kind | ""
global.certManager.issuerRef.group | provide certManager issuer group | ""
registry | controller container image repository| csf-docker-delivered.repo.cci.nokia.net
internalCitmRegistry | internal ingress controller container image registry | "csf-docker-delivered.repo.cci.nokia.net"
internalRsyslogClientRegistry | internal rsyslog-client container image registry | "csf-docker-delivered.repo.cci.nokia.net"
...
imageFlavor | set image flavor at root level, available options "rocky8" | ""
imageFlavorPolicy | set image flavor policy at root level, available options - "Strict, BestMatch" | ""
controller.name | name of the controller component | controller 
controller.imageRepo | controller image repository name | citm
controller.imageName | controller image name | citm-nginx-ingress
controller.imageTag | controller container image tag | 1.24.0-1.4.2-1.1.0
controller.imageFlavor | set image flavor at workload level, available options "rocky8" | ""
controller.imageFlavorPolicy | set image flavor policy at workload level, available options - "Strict, BestMatch" | ""
...
controller.gracefulShutdownSleepSeconds | Sleep to wait for any kubernetes-related race conditions to pass before initiating a graceful shutdown | 5
controller.preStop.command | This command will be used as part of lifecycle preStop hook , `/bin/sh -c '<USER_COMMAND>'` | ""
controller.terminationGracePeriodSeconds | Grace period applies to the total time it takes for both the PreStop hook to execute and for the Container to stop normally | 60
controller.pdb.enabled | Pod disruption budget will define minAvailable or maxUnavailable when there is disruption. Enable this flag to enable PodDistuptionBudget. | true
controller.pdb.minAvailable | Minimum number of pods from the set that must still be available after the eviction. It can be either an absolute number or a percentage. | 1
controller.pdb.maxUnavailable | Specify the number of pods that can be unavailable after the eviction. It can be either an absolute number or a percentage. | 0
...
controller.disableIvp4 | (Deprecated) disable Ipv4  for nginx.<br>See [disable-ipv4](docker-ingress-configmap.md#disable-ipv4)| none
controller.disableIpv4 | disable Ipv4  for nginx.<br>See [disable-ipv4](docker-ingress-configmap.md#disable-ipv4)| false
controller.disableIvp6 | (Deprecated) disable Ipv6  for nginx.<br>See [disable-ipv6](docker-ingress-configmap.md#disable-ipv6)| none
controller.disableIpv6 | disable Ipv6  for nginx.<br>See [disable-ipv6](docker-ingress-configmap.md#disable-ipv6)| false
...
controller.disableRootAliasDirective | Disables/Enables usage of root alias directive | true
controller.strictValidatePathType | Disables/Enables strict validation of path field in ingress resource | false
controller.enableAnnotationValidations | Enables validation of annotations in ingress resource | true
...
[controller.defaultSSLCertificate] (Deprecated) (#default-certificate) |  Optionally specify the secret name for default SSL certificate. Must be namespace/secret_name. See also (#cert-manager) | ""
controller.defaultSSLCertificateTLS.keyNames.caCrt  | (Deprecated) defaultSSLCertificate secret ca.crt custom key name  | "ca.crt"
controller.defaultSSLCertificateTLS.keyNames.tlsKey | (Deprecated) defaultSSLCertificate secret tls.key custom key name **Mandatory** | "tls.key"
controller.defaultSSLCertificateTLS.keyNames.tlsCrt | (Deprecated) defaultSSLCertificate secret tls.crt custom key name **Mandatory** | "tls.crt"
[controller.defaultStreamSSLCertificate](#default-stream-certificate) | (Deprecated) Optionally specify the secret name for default Stream SSL certificate. Must be namespace/secret_name. See also (#cert-manager) | ""
controller.defaultStreamSSLCertificateTLS.keyNames.caCrt  | (Deprecated) defaultStreamSSLCertificate secret ca.crt custom key name  | "ca.crt"
controller.defaultStreamSSLCertificateTLS.keyNames.tlsKey | (Deprecated) defaultStreamSSLCertificate secret tls.key custom key name **Mandatory** | "tls.key"
controller.defaultStreamSSLCertificateTLS.keyNames.tlsCrt | (Deprecated) defaultStreamSSLCertificate secret tls.crt custom key name **Mandatory** | "tls.crt"
[controller.tls.sslCertificate.secretRef.name] (#default-certificate) |  Optionally specify the secret name for SSL certificate. Must be namespace/secret_name. See also (#cert-manager) | ""
controller.tls.sslCertificate.secretRef.keyNames.caCrt  | sslCertificate secret ca.crt custom key name  | "ca.crt"
controller.tls.sslCertificate.secretRef.keyNames.tlsKey | sslCertificate secret tls.key custom key name **Mandatory** | "tls.key"
controller.tls.sslCertificate.secretRef.keyNames.tlsCrt | sslCertificate secret tls.crt custom key name **Mandatory** | "tls.crt"
[controller.tls.streamSSLCertificate.secretRef.name](#default-stream-certificate) | Optionally specify the secret name for Stream SSL certificate. Must be namespace/secret_name. See also (#cert-manager) | ""
controller.tls.streamSSLCertificate.secretRef.keyNames.caCrt  | streamSSLCertificate secret ca.crt custom key name  | "ca.crt"
controller.tls.streamSSLCertificate.secretRef.keyNames.tlsKey | streamSSLCertificate secret tls.key custom key name **Mandatory** | "tls.key"
controller.tls.streamSSLCertificate.secretRef.keyNames.tlsCrt | streamSSLCertificate secret tls.crt custom key name **Mandatory** | "tls.crt"
controller.certificate.api | set certManager api version  | ""
controller.certificate.issuerRef.name | provide certManager issuer name | ""
controller.certificate.issuerRef.kind | provide certManager issuer kind | ""
controller.certificate.issuerRef.group | provide certManager issuer group | ""
controller.certificate.duration | set certificate expiration time | 8760h
controller.certificate.renewBefore | set time before the expiration time when the certificate needs to be renewed | 360h
controller.certificate.secretName | set a name for the secret to be created | ""
controller.certificate.subject | set the subject fields for the certificate | ""
controller.certificate.commonName | set common name field for the certificate | ""
controller.certificate.usages | set usages field for certificate, defaults to "server auth" if not provided | ""
controller.certificate.dnsNames | set the dnsNames to be accepted by the certificate |  ""
controller.certificate.uris | set uris that have to accepted by the certificate | ""
controller.certificate.ipaddresses | list of ipaddresses to be accepted by the certificate | ""
controller.certificate.privateKey.algorithm | algorithm to be used by the private key | ""
controller.certificate.privateKey.encoding | encoding format for the private key | ""
controller.certificate.privateKey.size | set the key size | ""
controller.certificate.privateKey.rotationPolicy | set rotation policy for the private key | ""
...
controller.disableTlsForStatusPort | Enables or disables HTTPS connection for the status port | false
controller.addStatusPortToHostports | enable/disable addition of status port to host port section | true
...
controller.clusterDomain | If needed to communicate with an IP address which is outside of the hosted Kubernetes cluster, then it's suggested the IP to be exposed using clusterDomain | cluster.local
...
controller.enableSystemId | Generate systemid in json log | true
...
controller.enableCustomAlerts | Enable the custom alerts for the UK-TSR 8.1 and UK-TSR 8.5 | true
controller.customUnhardenedAlertMsg | User can set custom alert message for UK-TSR 8.1 Alerts | ""
...
controller.evenPodSpreadEnabled | allows user to ensure that EvenPodSpread feature gate is enabled | false
...
controller.replicasManagedByHPA | Set whether HPA to control the replica count or use user provided replicaCount | false
controller.resources | controller pod resource requests & limits | {}
controller.ephemeralVolume.emptyDir.medium | emptyDir medium to use blank for disk storage and Memory for in memory, with size limit(set at emptyDir.sizeLimit)  beyond which pod eviction will happen(controller.ephemeralVolume should be disabled) | empty
controller.ephemeralVolume.emptyDir.sizeLimit | Set the size limit for the emptry dir volume | "1Gi"
...
controller.ephemeralVolume.generic.resources.requests.ephemeral-storage | set resource requests for the storge | "200Mi"
controller.ephemeralVolume.generic.resources.limits.ephemeral-storage | set resource requests for the storge | "1Gi"
...
controller.service.externalTrafficPolicy | denotes if this Service desires to route external traffic to node-local or cluster-wide endpoints. There are two available options: Cluster (default) and Local. | ""
controller.service.healthCheckNodePort | specifies the healthcheck nodePort for the service. This only applies when type is set to LoadBalancer and externalTrafficPolicy is set to Local. If a value is specified, is in-range, and is not in use, it will be used.  If not specified, a value will be automatically allocated.  External systems (e.g. load-balancers) can use this port to determine if a given node holds endpoints for this service or not.  If this field is specified when creating a Service which does not need it, creation will fail. This field will be wiped when updating a Service to no longer need it (e.g. changing type). This field cannot be updated once set or can be updated to the same value. | 0
controller.service.dualStack.enabled | enable or disable dual stack feature | false
controller.service.dualStack.ipFamilyPolicy | set the behavior of dual stack among SingleStack, PreferDualStack, RequireDualStack | "RequireDualStack"
controller.service.dualStack.ipFamilies | set ip families to be assigned to the resource | ["IPv6", IPV4"]
...
controller.probe.enabled | enable or disable probe times | true
controller.probe.startup.path | path to send probe requests | /healthz
controller.probe.startup.delay | inital delay seconds | 10
controller.probe.startup.period | probe request interval | 10
controller.probe.startup.timeout | timeout interval seconds | 3
controller.probe.startup.maxfailure | max number  of pod failures | 6
controller.probe.liveness.exec | command to check for nginx health | ["/bin/bash", "-c", 'stat /nginx-config/nginx.pid && ps `cat /nginx-config/nginx.pid`']
controller.probe.liveness.delay | initial delay seconds | 10
controller.probe.liveness.period | probe request interval | 10
controller.probe.liveness.timeout | timeout interval seconds | 3
controller.probe.liveness.maxfailure | max number of pod failures | 5
controller.probe.readiness.path | path to send probe requests | /
controller.probe.readiness.delay | intial delay seconds | 10
controller.probe.readiness.period | probe request interval | 5
controller.probe.readiness.timeout | timeout interval seconds | 3
controller.probe.readiness.maxfailure | max number of pod failures | 5
controller.hpa.enabled |  enable or disable horizontal pod autoscaling feature. If HPA is enabled replicaCount will be ignored |
controller.hpa.maxReplicas | maximum no.of pods hpa can scale up to | 3
controller.hpa.minReplicas | minimum no.of pods hpa can scale down to | 1
controller.hpa.predefinedMetrics.enabled | enable or disable predefined (cpu and memeory )  metrics  | true
controller.hpa.predefinedMetrics.averageCPUThreshold | Average cpu usage target for metrics | 80
controller.hpa.predefinedMetrics.averageMemoryThreshold | Average memory usage target for metrics | 80
controller.hpa.behavior.scaleDown.stabilizationWindowSeconds | stabilization window time (secs) to consider before downscaling the deployment due to fluctuations | 60
controller.hpa.behavior.scaleUp.stabilizationWindowSeconds | stabilization window time (secs) to consider before upscaling the deployment due to fluctuations | 15
controller.hpa.behavior.scaleUp.policies | set behaviour for pod scale up | []
controller.hpa.behavior.scaleDown.policies | set behaviour for pod scale down | []
controller.hpa.behavior.scaleDown.selectPolicy | set the scale down policy to max/min  | Max
controller.hpa.behavior.scaleUp.selectPolicy | set the scale up policy to max/min  | Max
controller.hook.resources | set the resource requests and limits | {}
controller.hook.containerSecurityContext.dropCapabilities | drop capabilities | "ALL"
controller.hook.containerSecurityContext.runAsNonRoot | run container as non root | true
controller.hook.containerSecurityContext.secccompProfile.type | RuntimeDefault or Localhost seccompProfile | RuntimeDefault
controller.hook.containerSecurityContext.secccompProfile.path | path for Localhost profile | ""
controller.hook.containerSecurityContext.privilegeEscalation | Allow privilege escalation | false
controller.hook.containerSecurityContext.seLinuxOptions.enabled | enable or disable seLinux options | false
controller.hook.containerSecurityContext.seLinuxOptions.level | set the level | ""
controller.hook.containerSecurityContext.seLinuxOptions.role | set the role | ""
controller.hook.containerSecurityContext.seLinuxOptions.type | set the type | ""
controller.hook.containerSecurityContext.seLinuxOptions.user | set the user | ""
tests.skipTest | enable or disable helm test | false
...
|securityContext.seccompProfile.type | RuntimeDefault or Localhost seccompProfile | RuntimeDefault
|securityContext.secccomProfile.path | set the path for Localhost seccompProfile | ""
|securityContext.seLinuxOptions.enabled | enable or disable seLinuxOptions | false
|securityContext.seLinuxOptions.level | set the level | ""
|securityContext.seLinuxOptions.role | set the role | ""
|securityContext.seLinuxOptions.type | set the type | ""
|securityContext.seLinuxOptions.user | set the user | ""
[certManager.enabled](#cert-manager) |  Use cert-manager service for generating default certificate | ''
[certManager.used](!! Deprecated)(#cert-manager) | Use cert-manager service for generating default certificate | ""
...
istio.version | Istio version available in the cluster. For release upper or equal to 1.5, you can keep 1.5 or specify the exact version. There is only specific setting for Istio 1.4 | 1.17.3
...
[grafanaTools.resources](#grafana) | grafana-dashboard container resource requests & limits | {}
[grafanaTools.containerSecurityContext.dropCapabilities](#grafana) | drop capabilities | "ALL"
[grafanaTools.containerSecurityContext.runAsNonRoot](#grafana) | run container as non root | true
[grafanaTools.containerSecurityContext.secccompProfile.type](#grafana) | RuntimeDefault or Localhost seccompProfile | RuntimeDefault
[grafanaTools.containerSecurityContext.secccompProfile.path](#grafana) | path for Localhost profile | ""
[grafanaTools.containerSecurityContext.privilegeEscalation](#grafana) | Allow privilege escalation | false
[grafanaTools.containerSecurityContext.seLinuxOptions.enabled](#grafana) | enable or disable seLinux options | false
[grafanaTools.containerSecurityContext.seLinuxOptions.level](#grafana) | set the level | ""
[grafanaTools.containerSecurityContext.seLinuxOptions.role](#grafana) | set the role | ""
[grafanaTools.containerSecurityContext.seLinuxOptions.type](#grafana) | set the type | ""
[grafanaTools.containerSecurityContext.seLinuxOptions.user](#grafana) | set the user | ""
...
default404.istio.cni.enabled | Whether istio cni is enabled in the environment | true
...
#### cert-manager
citm-ingress supports cert-manager. You can use cert-manager for specifying default ssl certificate and default-stream-ssl-certificate.
if tls certificates are provided using either tls block or defaultSSLCertificate, defaultStreamSSLCertificate they will take precedence.
```
## Deprecated - use the approach specified below with certManager.enabled 
$ helm install citm-ingress --set certManager.used=true,certManager.servername=foo.bar.com,certManager.issuerRef.name="issuername",certManager.issuerRef.kind="issuerkind",certManager.issuerRef.group="issuergroup"
```

```
$ helm install citm-ingress --set certManager.enabled=true,certManager.servername=foo.bar.com
```
or using an input file
```
$ cat input.yaml 
certManager:
  # Deprecated - use the enabled flag instead
  used: true
  enabled: true
  ...
  dnsNames: 
  - "*.foo.bar.com"
  # if used flag is set instead of enabled flag provide the below details as well
  issuerRef:
    name: "issuername"
	kind: "issuerKind"
	group: "issuerGroup"
...
```
$ helm install citm-ingress --set controller.tls.sslCertificate.secretRef.name=default/mysecret
```
...
controller:
  tls:
    sslCertificate:
      secretRef:
        name: "{{ .Release.Namespace }}/mysecret"
```
...
By default, CITM ingress controller provide a default Fake certificate, self signed. You can use [cert manager](#cert-manager) or create your own certificate and pass the same(in the form of namespace/secret) for the streamSSLCertificate(defaultStreamSSLCertificate - deprecated ) during installation.

```
helm install citm-ingress --set controller.tls.streamSSLCertificate.secretRef.name=default/mysecret
```
...
Parameter | Description | Default
--------- | ----------- | -------
imageRepo | Repo of the rsyslog client sidecar image | citm
imageName | Rsyslog client sidecar image | citm-rsyslog-client
imageTag | Tag of the rsyslog client sidecar image | 1.0.0
imageFlavor | set image flavor at workload level, available options "rocky8" | ""
imageFlavorPolicy | set image flavor policy at workload level, available options - "Strict, BestMatch" | ""
...
containerSecurityContext.dropCapabilities | drop capabilities | "ALL"
containerSecurityContext.runAsNonRoot | run container as non root | true
containerSecurityContext.secccompProfile.type | RuntimeDefault or Localhost seccompProfile | RuntimeDefault
containerSecurityContext.secccompProfile.path | path for Localhost profile | ""
containerSecurityContext.privilegeEscalation | Allow privilege escalation | false
containerSecurityContext.seLinuxOptions.enabled | enable or disable seLinux options | false
containerSecurityContext.seLinuxOptions.level | set the level | ""
containerSecurityContext.seLinuxOptions.role | set the role | ""
containerSecurityContext.seLinuxOptions.type | set the type | ""
containerSecurityContext.seLinuxOptions.user | set the user | ""
...
probe.startup.command | command to send probe requests | ["/bin/bash", "-c", "pgrep rsyslogd && pgrep supercronic"]
...
probe.liveness.command | command to send probe requests | ["/bin/bash", "-c", "pgrep rsyslogd && pgrep supercronic"]
...
probe.readiness.command | command to send probe requests | ["/bin/bash", "-c", "pgrep rsyslogd && pgrep supercronic"] 
