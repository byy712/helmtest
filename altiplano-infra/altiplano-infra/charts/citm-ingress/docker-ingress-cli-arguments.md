...
| `--citm-allow-certificate-not-found`| Defines if a ingress certificate is not found, default certificate is used. Setting this to false will respond with a HTTP 403 (access denied) (default true) |
| `--citm-allow-invalid-certificate `| Defines if a ingress certificate is present but invalid, default certificate is used. Setting this to false will respond with a HTTP 403 (access denied) (default true) |
| `--citm-enable-https-all-server`   | Defines if https is activated even when no TLS section is provided |
| `--citm-ssl-passthrough-split`     | Activate ssl-passthrough on another port |
| `--citm-use-service-on-stream`     | Defines if on UDP/TCP service, request are forwarded to k8s service instead of backends |
